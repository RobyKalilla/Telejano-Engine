...
void Sbar_DrawPic (int x, int y, qpic_t *pic)
{
	if (cl.gametype != GAME_DEATHMATCH)
		Draw_AlphaPic (x + ((vid.width - 320)>>1), y + (vid.height-SBAR_HEIGHT), pic, sbar_alpha.value);
	else
		Draw_AlphaPic (x /* + ((vid.width - 320)>>1)*/, y + (vid.height-SBAR_HEIGHT), pic, sbar_alpha.value);
}

void Sbar_DrawPic2 (int x, int y, qpic_t *pic, float alpha)
{
	if (cl.gametype != GAME_DEATHMATCH)
...
// time
	minutes = cl.time * 0.0166666666;	// Tomaz - Speed
	seconds = cl.time - 60*minutes;
	tens = seconds * 0.1;	// Tomaz - Speed
...
// weapons
	for (i=0 ; i<7 ; i++)
	{
		if (cl.items & (IT_SHOTGUN<<i) )
		{
			...
			Sbar_DrawPic2 (i*24, -16, sb_weapons[flashon][i],sbar_weaponsalpha.value );//no alpha
...
// PGM 01/19/97 - team color drawing
	...
	if (cl.stats[STAT_HEALTH] >= 100)
		...
		f = cl.stats[STAT_HEALTH] * 0.05;	// Tomaz - Speed
...
// draw the text
	...
	x = 324;
	for (/* */; i < scoreboardlines && y < (signed)vid.height - 8 ; i++)
	{
		k = fragsort[i];
		s = &cl.scores[k];
		if (!s->name[0])
...
void Sbar_IntermissionOverlay (void)
{
	...
	// Tomaz - Centered Scoreboard Begin
	pic = Draw_CachePic ("gfx/complete.lmp");
	Draw_Pic ((vid.width/2)-94, (vid.height/2) - 96, pic);

	pic = Draw_CachePic ("gfx/inter.lmp");
	Draw_Pic ((vid.width/2)-160, (vid.height/2) - 64, pic);

	dig = cl.completed_time/60;
	Sbar_IntermissionNumber (vid.width/2, (vid.height/2) - 56, dig, 3, 0);
	num = cl.completed_time - dig*60;
	Draw_Pic ((vid.width/2)+74 ,(vid.height/2)- 56,sb_colon);
	Draw_Pic ((vid.width/2)+86 ,(vid.height/2)- 56,sb_nums[0][num/10]);
	Draw_Pic ((vid.width/2)+106,(vid.height/2)- 56,sb_nums[0][num%10]);

	Sbar_IntermissionNumber ((vid.width/2), (vid.height/2)- 16, cl.stats[STAT_SECRETS], 3, 0);
	Draw_Pic ((vid.width/2)+72,(vid.height/2)- 16,sb_slash);
