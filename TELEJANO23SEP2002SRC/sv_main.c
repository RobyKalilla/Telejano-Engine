...
#if 1 //Old debug version, safe version
...
// send over all entities (excpet the client) that touch the pvs
	...
	//	return;
		...
		if (msg->maxsize - msg->cursize < 16)
		{
			Con_Printf ("packet overflow for %s:%s, at %d\n", (pr_strings + ent->v.classname),(pr_strings + ent->v.model),msg->cursize );
...
// send an update
		...
		//ED_Print (EDICT_NUM(e));//Tei debugg
		...
		else
		{
			...
			{
				miss = ent->v.origin[i] - ent->baseline.origin[i];
				if ( miss < -(sv_dismin.value) || miss > (sv_dismin.value) ) //Tei adjustement from 0.1
					bits |= U_ORIGIN1<<i;
			}
		}
#endif 
...
#if 1
		if (singleplayer)//Tei SPE for telejano
		{
		...
		if (ent->v.effects3 == EF3_NOLERP )
			bits |= U_NOLERP;	// don't mess up the animation


		if (ent->baseline.colormap != ent->v.colormap)//SPE for telejano
		{
			if (singleplayer) // when no coop or deathmach
			{ 
				k = ent->v.colormap; //direct copy
		...
		if (ent->baseline.frame != ent->v.frame)
		{
			if (singleplayer) //SPE for telejano
		...
		{
			if (ent->baseline.effects2 != ent->v.effects2)
				bits |= U_EFFECTS2;
			if (ent->baseline.effects3 != ent->v.effects3)
				bits |= U_EFFECTS3;
		}
		//bits |= U_INFOVEL;
#endif
	...
	//
		...
		// Tei more fx
		if (bits & U_EFFECTS2)
			MSG_WriteByte (msg, ent->v.effects2);
		if (bits & U_EFFECTS3)
			MSG_WriteByte (msg, ent->v.effects3);
		// Tei more fx
...
extern int map_snapshot;	// Tomaz - MapShots
extern int map_snapname;	// Tomaz - MapShots
...
/*
	i = loadtextureimage2("focusx.pcx", false, false);//XFX

	if (!i)
		i = loadtextureimage2("focusx", false, false);//XFX

	if (!i)
		i = loadtextureimage2("gfx/focusx", false, false);//XFX

	if (!i)
		i = loadtextureimage2("gfx/focusx.pcx", false, false);//XFX

//	Con_Printf("Intentando cargar focusx.pcx en %d\n", i );
*/
...
//	
	...
	//ent->v.angles[0] = sv_stepsize.value;// Tei strange hack
