...
//#define CMDLINE_LENGTH	256 //standard
#define CMDLINE_LENGTH	1024 //Tei
...
specified, when a file is found by the normal search path, it will be copied
...
void MSG_WriteCoord (sizebuf_t *sb, float f)
{
	...
	//MSG_WriteFloat(sb, f*0.001);//Tei XFX BUG!!
}
...
void MSG_WriteAngle (sizebuf_t *sb, float f)
{
	...
	////MSG_WriteFloat(sb, f * 0.0001);//Tei XFX BUG!!
...
void MSG_BeginReading (void)
{
	msg_readcount = 0;
	msg_badread = false;
	Con_DPrintf("Reseted reading counter...\n");//Tei debugg
}

#if 1 //Tei new version, work in singleplayer, but not in as client
...
#else
int MSG_ReadByte (void) //old slow version
{
	int     c;
...
{
	return MSG_ReadShort() * (0.125f);	// Tomaz Speed
	//return MSG_ReadFloat()*1000;// Tei XFX bug!! * (0.125f);	// Tomaz Speed
}

float MSG_ReadAngle (void)
{
	return MSG_ReadChar() * (1.40625f);	// Tomaz Speed
	//return MSG_ReadFloat()*10000;//Tei XFX bug!!
}
...
============
*/
#if 0 //LordHavoc version
char    *va(char *format, ...)
{
	va_list         argptr;
	//static char             string[1024];
	static char             string[4096];// Tei weak function make more solid but slow
...
void COM_AddGameDirectory (char *dir)
{
	...
	// Tomaz - ANY Pak File End

	strcpy (com_gamedir, dir);
	Cvar_Set("gamedir", com_gamedir);//dave - gamedir readable cvar
...
//
	...
	// Tei - ANY T^PAK File Begin
	sprintf (dirstring, "%s/*.tpak", dir);
	handle = _findfirst (dirstring, &fileinfo);
	if (handle != -1)
	{
		do
...
char	moddir[255];//Tei big because crash unspected.was 32
