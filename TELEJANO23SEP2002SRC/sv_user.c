...
void SV_SetIdealPitch (void)
{
	...
	angleval = sv_player->v.angles[YAW] * 6.283186307179586 / 360;	// Tomaz Speed
	sinval = sin(angleval);
	cosval = cos(angleval);
...
==============
*/
cvar_t	sv_maxspeed = {"sv_maxspeed", "320", true, true};//Tei gamers request
...
//	accelspeed = sv_accelerate.value * host_frametime;
	...
	VectorMA (velocity, accelspeed, wishveloc, velocity);
}
...
// hack to not let you back into teleporter
	...
	wishvel[2] = forward[2]*fmove + right[2]*smove;
...
// read current angles	
	...
	if (impaim.value)
	{
		angle[0] = MSG_ReadFloat ();
		angle[1] = MSG_ReadFloat ();
		angle[2] = MSG_ReadFloat ();
	}
	else
	{
...
//#ifdef CUSTOM_QUAKE
	...
	{
		if (move->forwardmove)
			host_client->edict->v.msg_forwardmove =  move->forwardmove;
		if (move->sidemove)
			host_client->edict->v.msg_sidemove    =  move->sidemove;
		if (move->upmove)
...
nextmsg:
		...
		while (1)
		{
			...
			default:
				Sys_Printf ("SV_ReadClientMessage: unknown command char, val %d\n", cmd);
				return false;
