...
// decide which clipping hull to use, based on the size
	...
	{	// explicit hulls in the BSP model
		...
		{
			if (size[2] < 33) // pick the nearest of 32 or 56
				hull = &model->hulls[3]; // 32x32x32
			else
...
===============
*/
void Collision_Init (void);//Tei lh traces
...
//
	...
	{	// because movement is clipped an epsilon away from an actual edge,
		...
		// Tei - X unstuck - very experimental code
		/*
		ent->v.absmin[0] -= 1;
		ent->v.absmin[1] -= 1;
		ent->v.absmin[2] -= 1;
...
int xSV_RecursiveHullCheck (int num, double p1f, double p2f, double p1[3], double p2[3])
{
	dclipnode_t	*node;
	int			side;
	double		midf, mid[3];
...
====================
*/
void Old_SV_ClipToLinks ( areanode_t *node, moveclip_t *clip )
{
	link_t		*l, *next;
	edict_t		*touch;
	trace_t		trace;
...
// recurse down both sides
	...
	if ( clip->boxmaxs[node->axis] > node->dist )
		Old_SV_ClipToLinks ( node->children[0], clip );
	if ( clip->boxmins[node->axis] < node->dist )
		Old_SV_ClipToLinks ( node->children[1], clip );
}
