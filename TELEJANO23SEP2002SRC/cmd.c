...
// find a \n or ; line break
		...
		for (i=0 ; i< cmd_text.cursize ; i++)
		{
			...
			if( text[i] == '\n' && (quotes&1))//Tei support for long strings
				text[i] = ' ';
			else
			if (text[i] == '\n' ) 
				break;
...
void RemoveCRLF(char * f)
{
	...
	for (i=0;i<leng;i++)
	{
		...
		if( f[i] == 0x0d && (quotes&1))
				f[i] = ' ';
		else
		if( f[i] == 0x0a && (quotes&1))
				f[i] = ' ';
	}
};
...
void Cmd_Menu_f (void)
{
	...
	// Strange bug, for some reason that crack by error when not error ocurrer (?)
	if (Cmd_Argc () != 2)
	{
		Con_Printf ("menu <filename> : execute a menu file\n");
		Con_Printf (" Will automatically append .menu.txt as extension\n");
		return;
	}*/

	sprintf(filename,"%s.menu.txt",Cmd_Argv(1));//Generating filename

	mark = Hunk_LowMark ();
	f = (char *)COM_LoadHunkFile (filename);
	if (!f)
	{
//Tei retry to load a exec filename
		sprintf(filename,"%s/%s.menu.txt",com_gamedir,Cmd_Argv(1));//Generating filename
		f = (char *)COM_LoadHunkFile (filename);
		if (!f)
		{
...
#else
	char		*cmd_argv[MAX_ARGS];//Tei xfx
#endif
...
//
	...
	Cmd_AddCommand ("menu",Cmd_Menu_f);//Tei
...
#if 1 //Telejano longlines support
		if (*text == '"')
			quotes = !quotes;//Tei longline support  - XFX! 

		if (*text == '\n' && !quotes) //Tei long line support - XFX!
...
void Cmd_ForwardToServer (void)
{
	...
	Con_DPrintf ("Sending cmd '%s' to server...\n", Cmd_Argv(0));
